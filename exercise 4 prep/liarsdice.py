    ...
    for player_number in range(0, players):
        ...
        for face, total in enumerate(dice_array):
            if total != 0:
                ...
                session.commit()
    print ("Game started! Your id is " + str(game.id) + " and your password is " +
...
def getDice(game_id, player_number):
    ...
    dice = (session.query(Dice)
               ...
               .all())
    print ("Dice for player " + str(player_number) + " in game " + str(game_id)
           + ".")
    for die in dice:
...
def raiseBid(game_id, die_face, die_total):
    ...
    total increasing. Since this is a test example, there are no checks in place
    to make sure the raise bid isn't more then possible.
    On the other hand, that might be interesting for players not noticing that a
    bid isn't phyically capable of happening."""
...
def callLiar(game_id):
    ...
    print ("Player " + str(previous_player) + " said there were "
           + str(game.bid_number) + " dice with a face of " + str(game.bid_face)
           + " showing. The actual total was " + str(dice_total) + ".")
